...
在 `packages-web/theme/src/core/css-generator.ts` 中定义侧边栏背景变量：
...
```typescript
// packages-web/theme/src/adapters/sidebar-menu.ts
...
在 `packages-web/base/src/layout/Common/GlobalSider/index.scss` 中：
...
可以打开测试文件 `packages-web/theme/test/sider-dark-mode.html` 在浏览器中查看效果。
